...
class DialogPerformanceTester:
    ...
    async def setup_test_users(self, count: int) -> None:
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                successful_users.append((user_id, token, f"User{i} Test{i}"))
    ...
    async def run_dialog_tests(self) -> None:
        """Запуск тестов диалогов"""
        print(f"🔄 Тестирование диалогов...")
            ...
            for j in range(1, min(DIALOGS_PER_USER + 1, len(self.users))):
                ...
                if friend_idx != i:
                    ...
                    for msg_num in range(MESSAGES_PER_DIALOG):
                        message_text = f"Сообщение {msg_num + 1} от {name}"
                        send_tasks.append(self.send_message(token, friend_id, message_text))
        
        print(f"📤 Отправка {len(send_tasks)} сообщений...")
        ...
        print(f"✅ Успешно отправлено {successful_sends} сообщений")
        ...
        print(f"📥 Чтение {len(read_tasks)} диалогов...")
        ...
        for result in read_results:
            if isinstance(result, Exception):
                print(f"❌ Ошибка чтения сообщений: {result}")
            else:
        ...
        print(f"✅ Успешно прочитано {successful_reads} диалогов ({total_messages_read} сообщений)")
    
    def print_metrics(self) -> None:
        """Вывод метрик производительности"""
        print("\n" + "="*60)
        print("📊 МЕТРИКИ ПРОИЗВОДИТЕЛЬНОСТИ (PostgreSQL)")
        print("="*60)
    ...
    def save_metrics_to_file(self, filename: str = None) -> None:
        ...
        if filename is None:
            # Определяем бэкенд из переменной окружения
            backend = os.getenv("DIALOG_BACKEND", "postgresql").lower()
            filename = f"lesson-07/dialog_metrics_{backend}.json"
        
        # Определяем используемый бэкенд для метрик
        backend_name = os.getenv("DIALOG_BACKEND", "postgresql").upper()
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='Тестирование производительности диалогов')
    ...
    print("🚀 ТЕСТИРОВАНИЕ ПРОИЗВОДИТЕЛЬНОСТИ ДИАЛОГОВ")

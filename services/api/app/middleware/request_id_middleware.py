...
class RequestIdMiddleware(BaseHTTPMiddleware):
    """
    Middleware для обработки X-Request-ID header в монолитном приложении
    """
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        # Добавляем в логи
        logger.info(
            f"Monolith request started: {request.method} {request.url.path}",
            extra={
        ...
        logger.info(
            f"Monolith request completed: {response.status_code}",
            extra={
                "request_id": request_id,
...
def setup_logging_with_request_id():
    """Настройка логирования с поддержкой request_id для монолита"""
    
    class RequestIdFilter(logging.Filter):
        def filter(self, record):
            record.request_id = get_request_id() or 'no-request-id'
            record.service = getattr(record, 'service', 'monolith')
            return True
    ...
    formatter = logging.Formatter(
        '%(asctime)s - %(name)s - %(levelname)s - [%(service)s] [%(request_id)s] - %(message)s'
    )

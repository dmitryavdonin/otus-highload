...
USERS_BEFORE=$(docker-compose -f docker-compose-pg-replication.yml exec db-master psql -U postgres -d social_network -t -c "SELECT COUNT(*) FROM users;")
USERS_BEFORE=$(echo $USERS_BEFORE | tr -d ' ')
echo "Количество пользователей перед тестом: $USERS_BEFORE"

# Шаг 1: Запускаем нагрузку на запись (100 тысяч пользователей)
echo -e "\n${YELLOW}Запуск нагрузки на запись (100 тысяч пользователей):${NC}"
/home/busybox/dev/otus-highload/venv/bin/python /home/busybox/dev/otus-highload/lesson-01/load_test_replication.py localhost 100000 &
LOAD_PID=$!
...
docker-compose -f docker-compose-pg-replication.yml stop db-slave3
...
USERS_AFTER_PHASE1=$(docker-compose -f docker-compose-pg-replication.yml exec db-master psql -U postgres -d social_network -t -c "SELECT COUNT(*) FROM users;")
...
USERS_SLAVE1=$(docker-compose -f docker-compose-pg-replication.yml exec db-slave1 psql -U postgres -d social_network -t -c "SELECT COUNT(*) FROM users;")
...
USERS_SLAVE2=$(docker-compose -f docker-compose-pg-replication.yml exec db-slave2 psql -U postgres -d social_network -t -c "SELECT COUNT(*) FROM users;")
...
docker-compose -f docker-compose-pg-replication.yml exec -u postgres db-slave2 pg_ctl promote -D /var/lib/postgresql/data
...
IS_MASTER=$(docker-compose -f docker-compose-pg-replication.yml exec db-slave2 psql -U postgres -t -c "SELECT pg_is_in_recovery();")
...
echo -e "\n${YELLOW}Возобновление нагрузки на запись (50000 пользователей) на новый мастер:${NC}"
DB_PORT=5434 /home/busybox/dev/otus-highload/venv/bin/python /home/busybox/dev/otus-highload/lesson-01/load_test_replication.py localhost 50000 &
LOAD_PID=$!
...
USERS_AFTER_PHASE2=$(docker-compose -f docker-compose-pg-replication.yml exec db-slave2 psql -U postgres -d social_network -t -c "SELECT COUNT(*) FROM users;")
...
docker-compose -f docker-compose-pg-replication.yml start db-slave3
...
USERS_SLAVE3=$(docker-compose -f docker-compose-pg-replication.yml exec db-slave3 psql -U postgres -d social_network -t -c "SELECT COUNT(*) FROM users;")

...
echo -e "${BLUE}🚀 ТЕСТИРОВАНИЕ ДИАЛОГОВ С POSTGRESQL${NC}"
echo "=================================================================="
...
for i in {1..30}; do
    ...
    if [ $i -eq 30 ]; then
        ...
        echo -e "${YELLOW}💡 Запустите сервис командой: ./start_service.sh --backend postgresql${NC}"
        exit 1
    fi
...
export DIALOG_BACKEND=postgresql

echo -e "${GREEN}✅ Используется PostgreSQL бэкенд${NC}"
...
echo "  • Бэкенд: PostgreSQL"
echo "  • Результаты: lesson-07/dialog_metrics_postgresql.json"
echo ""

# Запускаем тест с PostgreSQL
python3 dialog_performance_test.py --users 40 --messages 20 --dialogs 10

echo ""
echo -e "${GREEN}🎉 Тестирование PostgreSQL завершено!${NC}"
echo -e "${BLUE}📊 Результаты сохранены в: lesson-07/dialog_metrics_postgresql.json${NC}" 